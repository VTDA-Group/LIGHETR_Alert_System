...
def make_visibility_figure(savedir, observatory, time, m, plot = True):
    """
    ...
    pupil = observatory.pupil
    fullpix = hp.query_strip(
        nside,
    ...
    t = astropy.time.Time(time,scale='utc',location=observatory)
    LST = t.sidereal_time('mean').deg
    phi_obs = ((self.pupil[:,1]+LST)%360)*np.pi/180
    theta_obs = (90-self.pupil[:,2])*np.pi/180
    ...
    p90i_obs_visible = None
    
    if sun_altaz.alt < -18*u.deg:
        p90i_obs_visible = np.intersect1d(p90i,newpix)
    
    if plot:
        if not os.path.exists(savedir+f"{observatory.name}_visibilities/"):
        ...
        mplot[newpix] = 0.2 #purple, color of HET pixels
        mplot[p90i] = 0.4 #blue, location of 90% region contours
        if sun_altaz.alt < -18*u.deg:
            mplot[p90i_obs_visible] = 0.5 #location of 90% region contours that are visible to HET while the sun is down
    ...
    return p90i_obs_visible

    


def prob_observable(m, observatory, header, time, savedir, plot = True):
# Determine resolution of sky map
    mplot = np.copy(m)
    npix = len(m)
    nside = hp.npix2nside(npix)
    ...
    plt.savefig(savedir+f"{observatory.name}_Full_Visibility.pdf")
